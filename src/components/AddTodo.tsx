        ...
        {(showOptions || text) && (
          <div className="px-5 pb-6 pt-5 border-t border-[var(--border-subtle)]">
            <div className="flex flex-wrap items-center gap-4">
              ...
              {/* Privacy toggle - pill style */}
              <button
                type="button"
                onClick={() => setIsPrivate(!isPrivate)}
                className={`inline-flex items-center gap-1.5 px-3.5 py-2 rounded-full border-2 transition-all cursor-pointer hover:shadow-sm ${
                  isPrivate
              ...
              >
                <Lock className={`w-4 h-4 flex-shrink-0 ${isPrivate ? 'text-purple-500' : 'text-[var(--text-muted)]'}`} />
                <span className={`text-sm font-medium ${isPrivate ? 'text-purple-600 dark:text-purple-400' : 'text-[var(--text-muted)]'}`}>
