...
const MentionAutocomplete = memo(function MentionAutocomplete({
  ...
  return (
    <motion.div
      ...
      className="absolute z-30 bg-[var(--surface-elevated)] border border-[var(--border)] rounded-[var(--radius-xl)] shadow-2xl overflow-hidden min-w-[180px] backdrop-blur-xl"
    ...
    >
      {filteredUsers.map((user) => (
        ...
        >
          <div
            className="w-8 h-8 rounded-[var(--radius-lg)] flex items-center justify-center text-[var(--text-inverse)] text-xs font-bold shadow-lg ring-1 ring-[var(--border-subtle)] group-hover:ring-[var(--border)] transition-all"
            style={{ backgroundColor: user.color }}
          >
            {user.name.charAt(0).toUpperCase()}
          </div>
          <span className="text-sm text-[var(--text-muted)] group-hover:text-[var(--foreground)] transition-colors">{user.name}</span>
        </button>
      ))}
    </motion.div>
  );
...
export const ChatInputBar = memo(function ChatInputBar({
      ...
      >
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="font-semibold text-[var(--foreground)]">Editing message</span>
          </div>
          <button
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-4 py-3 rounded-[var(--radius-xl)] border border-[var(--chat-border)] bg-[var(--chat-surface)] text-[var(--foreground)] text-sm focus:outline-none focus:border-[var(--accent)]/50 focus:ring-2 focus:ring-[var(--accent)]/20 transition-all"
            autoFocus
          />
          <motion.button
            onClick={handleSaveEditClick}
            disabled={!editText.trim()}
            className="px-5 py-3 bg-[var(--accent)] text-[var(--text-inverse)] rounded-[var(--radius-xl)] text-sm font-semibold hover:bg-[var(--accent)]/90 disabled:opacity-50 transition-all"
  ...
  return (
    <>
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="font-semibold text-[var(--foreground)]">{replyingTo.created_by}</span>
              </div>
              <button
      ...
      {/* Rate Limit Warning */}
      {rateLimitWarning && (
        <div className="px-4 py-2 bg-[var(--warning-light)] border-t border-[var(--warning)]/20 text-[var(--warning)] text-sm flex items-center gap-2">
      ...
      {/* Input Area */}
      <div className="p-4 border-t border-[var(--border)] bg-[var(--surface)] relative">
        ...
        <AnimatePresence>
          {showEmojiPicker && (
            <motion.div
              ...
              className="mb-3 bg-[var(--surface-elevated)] border border-[var(--border)] rounded-[var(--radius-2xl)] shadow-2xl overflow-hidden"
            >
                  ...
                  <button
                    ...
                    className={`flex-1 py-3 text-xs font-semibold capitalize transition-all duration-200 ${
                      emojiCategory === cat
                        ...
                        : 'text-[var(--chat-text-secondary)] hover:bg-[var(--chat-surface)] hover:text-[var(--text-muted)]'
                    }`}
                  >
                    {cat}
                  </button>
        ...
        <div className="flex items-end gap-2">
          <motion.button
            ...
            className={`p-2.5 min-w-[44px] min-h-[44px] flex items-center justify-center rounded-[var(--radius-xl)] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed ${
              showEmojiPicker ? 'bg-[var(--accent)]/20 text-[var(--accent)]' : 'hover:bg-[var(--chat-surface-hover)] text-[var(--chat-text-secondary)] hover:text-[var(--foreground)]'
            }`}
          ...
          <motion.button
            ...
            disabled={disabled}
            className="p-2.5 min-w-[44px] min-h-[44px] flex items-center justify-center rounded-[var(--radius-xl)] transition-all duration-200 hover:bg-[var(--chat-surface-hover)] text-[var(--chat-text-secondary)] hover:text-[var(--foreground)] disabled:opacity-50"
          ...
          <div className="flex-1 relative">
            <textarea
              ...
              className="w-full px-5 py-3 rounded-[var(--radius-2xl)] border border-[var(--chat-border)] bg-[var(--chat-surface)] text-[var(--foreground)] placeholder:text-[var(--chat-text-muted)] focus:outline-none focus:border-[var(--accent)]/50 focus:ring-2 focus:ring-[var(--accent)]/20 resize-none max-h-28 transition-all text-[15px] disabled:opacity-50 disabled:cursor-not-allowed"
              style={{
                height: 'auto',
                minHeight: '48px',
                maxHeight: '112px'
          ...
          <motion.button
            ...
            className="p-2.5 min-w-[44px] min-h-[44px] flex items-center justify-center rounded-[var(--radius-xl)] bg-[var(--accent)] text-[var(--text-inverse)] hover:bg-[var(--accent)]/90 disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-200"
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
