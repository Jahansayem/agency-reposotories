...
export function generateTaskSummary(
  ...
  lines.push('');
  lines.push('═══════════════════════════════════════');
  lines.push('Generated by Bealer Agency Todo App');
...
export function generateMarkdownSummary(
  ...
  lines.push('');
  lines.push('---');
  lines.push(`*Generated by Bealer Agency Todo App on ${format(now, "MMMM d, yyyy 'at' h:mm a")}*`);
...
export function generateJSONSummary(
  ...
  summary.generator = 'Bealer Agency Todo App';
...
// ============================================
// eAgent Contact Note Generator
// ============================================

/**
 * Human-readable labels for policy types used in eAgent notes
 */
const POLICY_TYPE_LABELS: Record<string, string> = {
  ...
  bundle: 'Bundle Policy',
};

/**
 * Generates a clean, customer-centric contact note formatted for
 * pasting into Allstate's eAgent CRM.
 *
 * Output is plain text with no ASCII art borders, no "Generated by"
 * footer, and no internal metadata (priority, created_at, agency_id).
 *
 * Sections included conditionally:
 * - Customer name (first line, skipped if absent)
 * - Contact reason (todo text + policy type if available)
 * - CALL NOTES (transcription, if present — most important content)
 * - AGENT NOTES (notes, if present)
 * - COMPLETED STEPS (only completed subtasks)
 * - Completion footer line
 */
export function generateEAgentNote(todo: Todo, completedBy: string): string {
  const lines: string[] = [];

  // Customer name — first line
  if (todo.customer_name) {
    lines.push(`Customer: ${todo.customer_name}`);
  }

  // Contact reason — task text + policy type
  let contactReason = todo.text;
  if (todo.policy_type && POLICY_TYPE_LABELS[todo.policy_type]) {
    contactReason += ` \u2014 ${POLICY_TYPE_LABELS[todo.policy_type]}`;
  }
  lines.push(`Contact Reason: ${contactReason}`);

  // CALL NOTES — transcription (most important content)
  if (todo.transcription) {
    lines.push('');
    lines.push('CALL NOTES');
    lines.push(todo.transcription);
  }

  // AGENT NOTES — free-form notes
  if (todo.notes) {
    lines.push('');
  ...
  // COMPLETED STEPS — only completed subtasks
  const completedSubtasks = todo.subtasks?.filter(s => s.completed) || [];
  ...
  // Completion footer
  const completionDate = safeFormatDate(
    new Date().toISOString(),
    'MMM d, yyyy'
  );
  lines.push('');
  lines.push(`Completed by ${completedBy} \u2014 ${completionDate}`);

  return lines.join('\n');
}

/**
 * Returns just the transcription text from a todo, for use with
 * a "Copy Call Notes Only" button. Returns an empty string if
 * no transcription exists.
 */
export function generateTranscriptionOnly(todo: Todo): string {
  return todo.transcription?.trim() || '';
}
